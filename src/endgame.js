...
class Endgame extends Phaser.Scene {
	...
	create(data) {
		...
		if (this.ranking[0] > this.ranking[1] || (this.ranking[0] == this.ranking[1] && this.players[0].deaths < this.players[1].deaths)) {	
			this.first_name = this.add.text(600, 250, this.players[0].name, { color: '#452600', fontSize: '45px', fontFamily: 'Metamorphous' });
			this.second_name = this.add.text(600, 350, this.players[1].name, { color: '#452600', fontSize: '45px', fontFamily: 'Metamorphous' });
		} else if (this.ranking[0] < this.ranking[1] || (this.ranking[0] == this.ranking[1] && this.players[0].deaths > this.players[1].deaths)) {
			this.first_name = this.add.text(600, 250, this.players[1].name, { color: '#452600', fontSize: '45px', fontFamily: 'Metamorphous' });
			this.second_name = this.add.text(600, 350, this.players[0].name, { color: '#452600', fontSize: '45px', fontFamily: 'Metamorphous' });
		}
	}
